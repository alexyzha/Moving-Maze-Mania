...
public class MazeObject {
    ...
    private static readonly Pair[] DIRECTIONS = new Pair[4] {
        new Pair(0,-2),
        new Pair(-2,0),
        new Pair(0,2),
        new Pair(2,0)
    ...
    public MazeObject(int x_, int y_, int seed_ = 0) {
    ...
    public void Shift() {
        ...
        Pair next = new Pair(origin_x+DIRECTIONS[next_dir].x,
                             origin_y+DIRECTIONS[next_dir].y);
        ...
        for(int i = 0; i < 4; ++i) {
            if((maze_frame[next.x,next.y] & (1<<i)) > 0) {
                ...
                Pair next_mid = GetMid(next.x,
                                       next.y,
                                       next.x+DIRECTIONS[i].x,
                                       next.y+DIRECTIONS[i].y);
        ...
        while(q.Count > 0) {
            ...
            for(int i = 0; i < 4; ++i) {
                ...
                if(!IsValid(next_x,next_y) ||
                   ((maze_frame[next_x,next_y] & IN_MAZE) == 0) ||
                   ((maze_frame[mid.x,mid.y] & WALL) > 0)) {
                    continue;
                }
    ...
    */
    ...
    private int RandomWalk() {
        // init
        Stack<Pair> current_walk = new Stack<Pair>();
    ...
    public void PrintMaze() {
        Console.Write("\x1B[32mMaze:\n");
        for(int i = 0; i < boundary_x; ++i) {
            for(int j = 0; j < boundary_y; ++j) {
                Console.Write(maze_frame[i,j] == 0x80 ? "0 " : "  ");
            }
            Console.Write("\n");
        }
    }
    ...
    private Pair GetMid(Pair a, Pair b) {
        int dx = (a.x-b.x)/2;
        int dy = (a.y-b.y)/2;
        return new Pair(a.x-dx,a.y-dy);
    }
